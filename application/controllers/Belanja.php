    ...
    function index()
    {
       ...
       $date = date('m');
        ...
        if($t != 'admin' || $t == null){ 
             ...
             $qq = "SELECT * ,count(*) as jml FROM `tbl_pesanan` a JOIN master_obat b ON a.nama_barang = b.nama_obat    where a.confirmed = 0 and MONTH(a.created_date) = '$date'  group by  a.kode_pesanan, a.pic_tertuju  UNION ALL
              SELECT *,count(*) as jml  FROM `tbl_pesanan_oot` a JOIN master_obat b ON a.nama_barang = b.nama_obat where a.confirmed = 0 and MONTH(a.created_date) = '$date' group by  a.kode_pesanan, a.pic_tertuju  UNION ALL
              SELECT * ,count(*) as jml FROM `tbl_pesanan_pre` a JOIN master_obat b ON a.nama_barang = b.nama_obat where a.confirmed = 0 and MONTH(a.created_date) = '$date' group by a.kode_pesanan, a.pic_tertuju";
              $datasp = $this->db->query($qq)->result();
                                  $data['sp'] = $datasp;

            $this->template->load('Depan', 'belanja/form_input', $data);

        }else {
            $qq = "SELECT *,count(*) as jml  FROM `tbl_pesanan` a JOIN master_obat b ON a.nama_barang = b.nama_obat where a.confirmed = 0 group by  a.kode_pesanan, a.pic_tertuju  UNION ALL
              SELECT *,count(*) as jml  FROM `tbl_pesanan_oot` a JOIN master_obat b ON a.nama_barang = b.nama_obat  where a.confirmed = 0 group by  a.kode_pesanan, a.pic_tertuju  UNION ALL
              SELECT *,count(*) as jml  FROM `tbl_pesanan_pre` a JOIN master_obat b ON a.nama_barang = b.nama_obat  where a.confirmed = 0 group by a.kode_pesanan, a.pic_tertuju";
              $datasp = $this->db->query($qq)->result();
             $data['sp'] = $datasp;
            if (!empty($_POST['dari']) && !empty($_POST['sampai']) && empty($_POST['pbf'])) {
                ...
                $this->template->load('template', 'belanja/lihat_data', $data);
    ...
    function post_konfirmasi()
    {
        if (isset($_POST['submit'])) {
                ...
                if($jenis == "AI"){
                  ...
                  $q =  $this->Model_pesanan->konfirmasi($data, $no_sp);

                }else if($jenis == "PRE"){
                     $q =  $this->Model_pesanan_pre->konfirmasi($data, $no_sp);



                }else if($jenis == "OOT"){
                     $q = $this->Model_pesanan_oot->konfirmasi($data, $no_sp);


                } 
                
                if($q){
                    $this->session->set_flashdata('msg', 'Pesanan Berhasil Konfirmasi');

                     redirect('Belanja'); 
                }else{
                    $this->session->set_flashdata('msg', 'Pesanan Gagal Konfirmasi');
    ...
    public function invoice_json()
    {
        ...
        $id = $row['id_invoice'];
          ...
          if ($level == 'admin' or $level == 'keuangan') {
                $nestedData[]    = "<a  class='btn btn-success  edit btn-sm' onclick='showDataObat($id)' data-target='#myModal' href='#modal-body' data-toggle='modal' ><i class='fa fa-edit'></i></a> | <a class='btn btn-danger btn-sm'  href='" . site_url('/belanja/delete/' .  $id) . "' id='HapusTransaksi'><i class='fa fa-trash'></i></a>";
            }
