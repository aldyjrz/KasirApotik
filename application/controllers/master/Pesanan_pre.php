<?php
class Pesanan_pre extends CI_Controller
{
    ...
    function add()
    {
        $m = date('m');
        $y = date('y');
        $get = $this->Model_pesanan_pre->get_max()->result();
        ...
        $data['record'] = $this->Model_pesanan_pre->tampil_data();
        $this->template->load('template', 'pesanan_pre/add_pesanan', $data);
    }
    public function detail_transaksi()
    {
        $this->load->model('Model_pesanan_pre');
        $id_penjualan = $_GET['term'];
        $dt['detail'] = $this->Model_pesanan_pre->cetak($id_penjualan)->result();

        $this->load->view('pesanan_pre/pesanan_history_detail', $dt);
    }
    ...
    function index()
    {
        ...
        $data['record'] = $this->Model_pesanan_pre->tampil_data();
        $this->template->load('template', 'pesanan_pre/lihat_data', $data);
    }

    function report()
    {
        if (isset($_POST['dari'])) {
            $data['record'] = $this->Model_pesanan_pre->get_laporan($_POST['dari'], $_POST['sampai'])->result();
    ...
    function post()
    {
        if ($_POST) {
            ...
            for ($i = 0; $i < $co; $i++) {
                ...
                $q =  $this->Model_pesanan_pre->crot($data);
            }
    ...
    function satuan()
    {
        ...
        $data['record']     =  $this->Model_pesanan_pre->get_satuan()->result();
    ...
    function edit()
    {
        if ($_POST) {
            ...
            $q =  $this->Model_pesanan_pre->edit($data, $id);
            ...
            $data['aa']     =  $this->Model_pesanan_pre->cetak($id)->result();
            $this->template->load('template', 'pesanan_pre/edit_pesanan', $data);
        }
    }
    function post_ajax()
    {
    ...
    function detail()
    {
        ...
        $data['record']     =  $this->Model_pesanan_pre->get_one($id)->result();
        $this->load->view('pesanan_pre/detail', $data);
    }

    function delete()
    {
        $id =  $_GET['kode'];
        $this->Model_pesanan_pre->delete($id);
    ...
    function delete_one()
    {
        ...
        $this->Model_pesanan_pre->delete_one($id);
        redirect('master/pesanan_pre/edit/?term=' . $kode);
    }
}