...
Baris += "</td>";
 	...
 	function AutoCompleteGue(Lebar, KataKunci, Indexnya) {
 		...
 		$.ajax({
 			url: "<?php echo site_url('master/Pesanan_pre/ajax_kode'); ?>",
 			type: "POST",
 			cache: false,
 	...
 	function SimpanTransaksi() {
 		...
 		$.ajax({
 			url: "<?php echo site_url('master/Pesanan_pre/post'); ?>",
 			type: "POST",
 			cache: false,
 			data: FormData,
 			dataType: 'json',
 			success: function(data) {
 				if (data.status == 1) {
 					alert(data.pesan);
 					window.location.href = "<?php echo site_url('master/Pesanan_pre'); ?>";
 				} else {
 					$('.modal-dialog').removeClass('modal-lg');
 					$('.modal-dialog').addClass('modal-sm');
