...
Baris += "</td>";
 	...
 	function AutoCompleteGue(Lebar, KataKunci, Indexnya) {
 		...
 		$.ajax({
 			url: "<?php echo site_url('Master/pesanan_oot/ajax_kode'); ?>",
 			type: "POST",
 			cache: false,
 	...
 	function SimpanTransaksi() {
 		...
 		$.ajax({
 			url: "<?php echo site_url('Master/pesanan_oot/post'); ?>",
 			type: "POST",
 			cache: false,
 			data: FormData,
 			dataType: 'json',
 			success: function(data) {
 				if (data.status == 1) {
 					alert(data.pesan);
 					window.location.href = "<?php echo site_url('Master/pesanan_oot'); ?>";
 				} else {
 					$('.modal-dialog').removeClass('modal-lg');
 					$('.modal-dialog').addClass('modal-sm');
